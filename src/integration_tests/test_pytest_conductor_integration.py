...
class TestPytestConductorIntegration:
    ...
    def test_build_and_install_package(self, project_root: Path, example_dir: Path):
    ...
    def test_basic_tag_ordering(self, example_dir: Path):
        ...
        test_lines = [line for line in output_lines if 'test_' in line and 'PASSED' in line]
        ...
        if fast_tests and slow_tests:
            ...
            assert first_fast_idx < first_slow_idx, "Fast tests should run before slow tests"
        ...
        if no_tag_tests:
            ...
            for line in test_lines:
                if 'fast' in line.lower() or 'slow' in line.lower():
                    ...
                    assert first_no_tag_idx < tagged_idx, "Untagged tests should run first"
    ...
    def test_unmatched_order_last(self, example_dir: Path):
        ...
        if no_tag_tests:
            ...
            for line in test_lines:
                if 'fast' in line.lower() or 'slow' in line.lower():
                    tagged_idx = test_lines.index(line)
                    assert tagged_idx < last_no_tag_idx, "Tagged tests should run before untagged tests"
    ...
    def test_mixed_tag_and_fixture_ordering(self, example_dir: Path):
        ...
        result, duration = self.run_pytest_with_logging(
            ["--tag-order", "fast", "slow", "--fixture-order", "basic_calculator", "advanced_calculator", "--ordering-mode", "mark", "-v"],
            example_dir,
        ...
        # In mark mode, fixture-order should be ignored, so this should work like tag ordering
        output_lines = result.stdout.split('\n')
        test_lines = [line for line in output_lines if 'test_' in line and 'PASSED' in line]
        ...
        if fast_tests and slow_tests:
            first_fast_idx = test_lines.index(fast_tests[0])
            first_slow_idx = test_lines.index(slow_tests[0])
            assert first_fast_idx < first_slow_idx, "Fast tests should run before slow tests in mark mode"

    def test_complex_ordering_scenario(self, example_dir: Path):
        """Test a complex ordering scenario with multiple tags and fixtures."""
        result, duration = self.run_pytest_with_logging(
            ["--tag-order", "unit", "integration", "fast", "slow", "--unmatched-order", "last", "-v"],
            example_dir,
        ...
        # Verify the order: unit tests first, then integration, then fast/slow, then untagged
        output_lines = result.stdout.split('\n')
        test_lines = [line for line in output_lines if 'test_' in line and 'PASSED' in line]
        ...
        if unit_tests and integration_tests:
            ...
            assert first_unit_idx < first_integration_idx, "Unit tests should run before integration tests"
        
        if integration_tests and no_tag_tests:
            last_integration_idx = test_lines.index(integration_tests[-1])
            first_no_tag_idx = test_lines.index(no_tag_tests[0])
            assert last_integration_idx < first_no_tag_idx, "Integration tests should run before untagged tests"
    ...
    def test_performance_comparison(self, example_dir: Path):
        ...
        ordered_result, ordered_duration = self.run_pytest_with_logging(
            ["--tag-order", "fast", "slow", "integration", "--unmatched-order", "last", "-v"],
            example_dir,
            "Ordered Execution"
        )
        ...
        overhead_percentage = (overhead / unordered_duration) * 100 if unordered_duration > 0 else 0
        ...
        assert overhead_percentage < 50, f"Ordering overhead too high: {overhead_percentage:.1f}%"
    ...
    def test_collection_with_ordering(self, example_dir: Path):
        ...
        if fast_collect and slow_collect:
            ...
            assert first_fast_idx < first_slow_idx, "Fast tests should be collected before slow tests" 